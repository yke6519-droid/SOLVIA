...
该模块只负责 Runtime 语义：创建 PendingInteraction、更新运行状态、分类用户
回复和发布 Runtime 事件。真正的等待、SSE 队列和 /reply 路由仍由注入的
AskUserBridge 负责，避免重复实现传输机制。
"""
...
class RuntimeInteractionService:
    ...
    def _apply_run_state(
        ...
        if interaction_state == InteractionState.CONFIRMED:
            context.state = AgentRunState.RUNNING
        elif interaction_state == InteractionState.CANCELLED:
            context.state = AgentRunState.CANCELLED
        else:
            context.state = AgentRunState.BLOCKED
