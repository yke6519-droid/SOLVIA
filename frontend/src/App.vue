...
<template>
  <div class="app-shell" :data-theme="theme">
    <header class="topbar">
      ...
      <div class="user-menu"><span class="status-dot"></span><span>{{ statusLabel }}</span><button class="theme-toggle" type="button" :aria-label="theme === 'dark' ? '切换浅色模式' : '切换深色模式'" :aria-pressed="theme === 'light'" @click="toggleTheme"><span class="theme-toggle-mark" aria-hidden="true"></span><span>{{ theme === 'dark' ? '浅色' : '深色' }}</span></button><span class="user-divider"></span><span class="user-name">测试用户</span><button class="icon-button" aria-label="打开设置">···</button></div>
    </header>
    ...
    <div class="workspace">
      ...
      <main class="main-panel">
        <section v-if="messages.length === 0" class="welcome-panel">
          ...
          <div class="welcome-copy"><p class="eyebrow">欢迎进入光伏运营工作台</p><h1 class="welcome-title"><span class="welcome-title-line welcome-title-left">把一个光伏问题，</span><span class="welcome-title-line welcome-title-right"><span class="inline-solar">交给阳光</span>和数据。</span></h1><p class="welcome-description">查询、预测、导入、复盘。用一句自然语言开始一项真实的运营任务。</p></div>
          <div class="shortcut-grid">
            <button class="shortcut-card" @click="useShortcut('查询英杰站今天的实际发电量')"><span class="shortcut-kicker">际数据</span><strong>查询发电量</strong><span>按站点和日期获取真实记录</span><span class="shortcut-arrow">↗</span></button>
        ...
        <div class="composer-wrap">
          <!-- <div class="composer-hint">
            ...
            </span>试试：预测英杰站明天的发电量，或查询哲丰一车间五月一日的实际数据
          </div> -->
            <div class="composer">
              <button class="composer-add" aria-label="添加文件" @click="showToast('文件导入将在 API 接入后启用')">+</button>
              <input v-model="input" type="text" placeholder="告诉我你想完成的光伏任务" @keydown.enter="sendMessage"/>
              <button v-if="isStreaming" class="stop-button" @click="isStreaming = false">停止</button>
              <button v-else class="send-button" aria-label="发送消息" @click="sendMessage">↗</button>
            </div>
            <div class="composer-foot">
              <span>SolarAgent 可能需要你确认关键业务条件</span>
              <span>Enter 发送</span>
            </div>
          </div>
      </main>
