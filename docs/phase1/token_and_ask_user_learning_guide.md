...
相关代码：[`app/routers/auth.py`](../../app/routers/auth.py)、[`app/services/user_service.py`](../../app/services/user_service.py)、[`app/services/auth_service.py`](../../app/services/auth_service.py)。
...
相关代码：[`app/dependencies/auth.py`](../../app/dependencies/auth.py)。
...
他的身份是真的，但这个会话未必属于他。因此 [`app/routers/sessions.py`](../../app/routers/sessions.py) 的 `_verify_session_ownership()` 会把 Token 解析出来的 `user_id` 与会话所属 `user_id` 比较：
...
当前内置验证页在 [`app/main.py`](../../app/main.py) 中：登录后保存 `data.access_token`，`apiFetch()` 给每个请求加上：
...
| 文件 / 组件 | 通俗职责 | 技术职责 |
| --- | --- | --- |
| [`predModels/Tools/ask_user_tool.py`](../../predModels/Tools/ask_user_tool.py) | 厨师按铃提问 | LangChain Tool，调用可替换的 `_input_handler(question)` |
| [`app/services/agent_manager.py`](../../app/services/agent_manager.py) | 前台按会话找对服务员 | 为每个 session 管理 Bridge，用 `ContextVar` 路由问题 |
| [`app/services/ask_user_bridge.py`](../../app/services/ask_user_bridge.py) | 传菜铃 + 等待椅 | `asyncio.Queue` 送问题，`threading.Event` 等回复 |
| [`app/routers/chat.py`](../../app/routers/chat.py) | 现场协调员 | SSE 输出事件，接收 `/reply`，校验会话归属 |
| [`app/main.py`](../../app/main.py) | 服务员 | 接收 SSE 的 `user_input_required`，展示弹窗，提交回答 |
...
当前 [`app/services/agent_manager.py`](../../app/services/agent_manager.py) 使用：
...
- 超时默认值来自 [`app/config.py`](../../app/config.py) 的 `ask_user_timeout = 120` 秒。
...
[`app/routers/chat.py`](../../app/routers/chat.py) 通过每会话一个 `asyncio.Lock` 保证：同一个会话尚在执行时，第二条消息直接返回 HTTP 409。
...
**结构**：将 `ask_user` 及调用它的 Agent/工作流改成 async；创建一个 `Future` 等待 `/reply` 来 `set_result()`。
...
1. `predModels/Tools/ask_user_tool.py`：先理解“提出问题”的最小接口。
2. `app/services/ask_user_bridge.py`：理解 Queue、Event、等待和唤醒。
3. `app/routers/chat.py`：理解问题如何变成 SSE、回复如何回到 Bridge。
4. `app/services/agent_manager.py`：最后理解 session 路由、ContextVar 和并发保护。
5. `app/main.py`：看前端如何监听事件与提交答案。
