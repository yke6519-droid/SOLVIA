...
当前 `Agent/tools.py` 中注册了 **20 个工具**。工具不是面向用户的菜单，而是 Agent 完成任务时可调用的“业务能力单元”。
...
flowchart TB
    ...
    subgraph api["FastAPI 应用层"]
        main["app/main.py<br/>应用、CORS、健康检查"]
...
| --- | --- | --- | --- |
| 前端层 | 当前 `app/main.py` 内置 HTML；未来 Vue | 用户如何登录、提问、看到流和弹窗 | 可信身份判定、数据库权限判断 |
| API 路由层 | `app/routers/` | 哪个接口接收什么请求，返回什么状态 | 写复杂预测算法、直接编排所有工具细节 |
| 认证与服务层 | `app/dependencies/`、`app/services/` | 当前是谁、会话属于谁、如何管理 Bridge / Agent | 把业务数据硬编码在内存里 |
| Agent 编排层 | `Agent/` | 如何选工具、带哪些上下文、如何记忆 | 直接暴露 HTTP、相信用户输入的身份 |
| 工具层 | `predModels/Tools/` | 如何真正查询、预测、导入、导出 | 处理浏览器 UI 状态 |
...
1. [`app/main.py`](../app/main.py)：应用入口、CORS、当前测试页。
2. [`app/routers/auth.py`](../app/routers/auth.py) 与 [`app/dependencies/auth.py`](../app/dependencies/auth.py)：登录和 Token 保护。
3. [`app/routers/chat.py`](../app/routers/chat.py)：SSE 事件、执行锁、AskUser 回复接口。
4. [`app/services/agent_manager.py`](../app/services/agent_manager.py) 与 [`app/services/ask_user_bridge.py`](../app/services/ask_user_bridge.py)：会话级 Agent 和人工交互桥接。
5. [`Agent/agent.py`](../Agent/agent.py)、[`Agent/prompt.py`](../Agent/prompt.py)、[`Agent/tools.py`](../Agent/tools.py)：Agent 如何组合。
6. [`Agent/memory.py`](../Agent/memory.py)：窗口、摘要、持久化记忆。
7. `predModels/Tools/`：按“查询 → 预测 → 导出 → 导入”的业务顺序阅读。
